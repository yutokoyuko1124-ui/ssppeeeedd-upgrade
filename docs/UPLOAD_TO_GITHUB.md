...
### iPadで「ファイルのアップロード」はできる？

できます。GitHubのWeb画面（Safari）で `Add file -> Upload files` から画像/JSON などを追加できます。
ただし、**ブランチ作成やPR作成のUI制限**で失敗する場合があるため、確実なのは次の運用です。

1. Working Copyで commit/push
2. GitHub本体で `work -> main` のPRを作成


## 「バイナリファイルはサポートされていません」と出る場合

Codex/一部連携のPR作成では、`png` などのバイナリ差分を含むと失敗することがあります。

対処方法（確実）:

1. ローカル（Working Copy など）で `git push -u origin work`
2. GitHub本体で `work -> main` のPRを作成

※ GitHub本体のPRはバイナリファイル（画像）を普通に扱えます。
